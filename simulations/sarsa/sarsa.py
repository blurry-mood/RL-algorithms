...
def test_minihack():
    ...
    sarsa = MyAgent(actions=list(range(env.action_space.n)),
                       alpha=ALPHA, gamma=GAMMA, eps=EPS)
    sarsa.load('sarsa_minihack')
...
def test_robot_warehouse():
    ...
    env = gym.make("rware:rware-tiny-2ag-v1", max_steps=MAX_STEPS,
                   n_agents=N_AGENTS, request_queue_size=1, sensor_range=1,
                   shelf_columns=3, column_height=0, shelf_rows=1)

    agents = [MyAgent(list(range(N_ACTIONS)), ALPHA, GAMMA, EPS)
              for _ in range(N_AGENTS)]
    for i, agent in enumerate(agents):
        agent.load(f'sarsa{i}')
